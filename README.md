...
Персональная авторизация (client_credentials):

1) В админке заполните `avito_client_id` и `avito_client_secret`.
2) Нажмите кнопку “Получить токен (client_credentials)” в админке.
3) Токен живёт 24 часа — при отправке сообщений он будет обновляться автоматически, если истёк.
